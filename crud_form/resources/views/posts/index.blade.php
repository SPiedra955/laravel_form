    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('posts.store') }}">
                ...
                <div class="mt-4">
                    ...
                    @if ($field === 'contenido')
                        <textarea name="{{ $field }}" id="{{ $field }}"
                            class="mt-1 p-2 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">{{ old($field) }}</textarea>
                    @else
            ...
            @foreach (['caducable', 'comentable'] as $checkbox)
                <div class="mt-4">
                    <label for="{{ $checkbox }}" class="block text-sm font-medium text-gray-700"></label>
            ...
            @foreach ($posts as $post)
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                        ...
                        <h1 class="mt-4 text-xl text-gray-900">{{ $post->titulo }}</h1>
                        <h1 class="mt-4 text-sm text-gray-700">{{ $post->extracto }}</h1>
                    </div>
                </div>
            @endforeach
        </div>
